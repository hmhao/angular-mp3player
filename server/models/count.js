...
AnalyticsCountSchema.statics.findHotArtist = function(cb){
    return this.aggregate(
        ...
        { $group: { _id: '$data.artist', total: {$sum: 1} }},
        { $project: { _id: 0 , artist: '$_id', total: '$count' }},
        { $sort: { total: -1}},
        { $limit: 10},
        cb);
};
...
/*歌手总量排行及日升降幅度*/
AnalyticsCountSchema.statics.findHotArtistRateByDate = function(cb){
    var time = new Date(),
    ...
    return this.aggregate(
        ...
        { $project: { _id: 0 , artist: '$_id', total: '$total', rate: {$subtract:['$today','$yesterday']} }},
        { $sort: { total: -1}},
        { $limit: 10},
        cb);
};
...
/*歌手总量排行及周升降幅度*/
AnalyticsCountSchema.statics.findHotArtistRateByWeek = function(cb){
    var time = new Date(),
        year = time.getFullYear(),
        month = time.getMonth(),
        date = time.getDate(),
...
/*歌手总量排行及月升降幅度*/
AnalyticsCountSchema.statics.findHotArtistRateByMonth = function(cb){
    var time = new Date(),
        year = time.getFullYear(),
        month = time.getMonth(),
...
/*歌手总量排行及季升降幅度*/
AnalyticsCountSchema.statics.findHotArtistRateByQuarter = function(cb){
    var time = new Date(),
        year = time.getFullYear(),
        month = time.getMonth(),
